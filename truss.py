...
class truss:
    ...
    def addElementByNode(self,nodeTable,nodeNr1,nodeNr2):
        ...
        #nodeTable = nodeTable.__init__(nodeNr1, nodeNr2)
        # nodeNr1 & nodeNr2 
    ...
    def getHighestNodeNr(self):
        # get the highest node number used in this truss structure
        # step 1: first generate a list of node numbers used, then return the highest value
        nodesUsed = []
        for x in self.elementList:
            nodesUsed.append(x.firstNode.nodeNr)
            nodesUsed.append(x.secondNode.nodeNr)
    ...
    def getTotalSurfaceArea(self,Unit):
        ...
        surfaceOffset = 200      # determined using SW
        surfaceGain = 7             # width of the segment
        totalSurfaceArea = 0
        for Elem in self.elementList:
            #add the surface to the totalSurfaceArea 
            totalSurfaceArea += (Elem.elementLength*surfaceGain*1000 + surfaceOffset)
        if Unit == '%':
